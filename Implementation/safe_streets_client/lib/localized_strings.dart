...
const _values = {
  ...
  AvailableStrings.LOGIN_LOGIN: "LOGIN",
  ...
  AvailableStrings.MISSING_STRING:
      "MISSING STRING. Please report to the developers",
  ...
  AvailableStrings.PARKING_ON_PEDESTRIAN_CROSSING: "Parking on pedestrian crossing",
...
///If the string is not found, this returns an error message.
String local(AvailableStrings string) {
  if (_values.containsKey(string)) return _values[string];
  return _values[AvailableStrings.MISSING_STRING] + " " + string.toString();
}

///This provides a method to return a localized string with a key that is defined at compile time.
String localKey(String key) {
  try {
    return local(
        AvailableStrings.values.firstWhere((e) => e.toString() == "AvailableStrings." + key));
  } catch (e) {
    return _values[AvailableStrings.MISSING_STRING] + " " + key;
  }
}
